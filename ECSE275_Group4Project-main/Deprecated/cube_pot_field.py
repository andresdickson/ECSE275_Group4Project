...
def compute_midpoint_DH(dh_params, joint_index, base_position):
    ...
    if joint_index < 1 or joint_index > len(dh_params):
        raise ValueError(
            "joint_index must be between 1 and the number of DH parameter sets."
        )
...
def plot_grid(end_effector1, end_effector2, target):
    ...
    plt.scatter(
        end_effector1[0], end_effector1[1], color="blue", label="End Effector 1", s=100
    )
    plt.scatter(
        end_effector2[0], end_effector2[1], color="green", label="End Effector 2", s=100
    )
...
if __name__ == "__main__":
    ...
    # Define Waypoint Object handles
    robot0_waypoint0 = sim.getObjectHandle("/waypoint0_0")
    robot0_waypoint1 = sim.getObjectHandle("/waypoint0_1")
    robot0_waypoint2 = sim.getObjectHandle("/waypoint0_2")
    robot1_waypoint0 = sim.getObjectHandle("/waypoint1_0")
    robot1_waypoint1 = sim.getObjectHandle("/waypoint1_1")
    robot1_waypoint2 = sim.getObjectHandle("/waypoint1_2")

    # pull waypoint coords
    robot0_waypoint0_world = np.array(get_position(sim, robot0_waypoint0))
    robot0_waypoint1_world = np.array(get_position(sim, robot0_waypoint1))
    robot0_waypoint2_world = np.array(get_position(sim, robot0_waypoint2))
    robot1_waypoint0_world = np.array(get_position(sim, robot1_waypoint0))
    robot1_waypoint1_world = np.array(get_position(sim, robot1_waypoint1))
    robot1_waypoint2_world = np.array(get_position(sim, robot1_waypoint2))

    # loops until both arms are close to the cube
    ##tweak threshold to come closer or further to the cube
    while (
        np.linalg.norm(robot0_world - cube0_world) > 0.05
        or np.linalg.norm(robot1_world - cube1_world) > 0.05
    ):
        print("1")
        ...
        alpha_att = 1.0
        beta_rep = 1.0
        F_total_0 = -alpha_att * F_att_0 - beta_rep * (
            F_rep_EE0 + F_rep_link1_0 + F_rep_link2_0 + F_rep_link3_0
        )
        # print(F_total_1)
        F_total_0[1] = 0
        F_total_1 = -alpha_att * F_att_1 - beta_rep * (
            F_rep_EE1 + F_rep_link1_1 + F_rep_link2_1 + F_rep_link3_1
        )
        F_total_1[1] = 0
        # plt.figure()
        ...
        if np.linalg.norm(robot0_world - cube0_world) < 0.01:
            robot0_joint_vel = np.zeros_like(robot0_joint_vel)
            robot1_joint_vel = np.matmul(J1.transpose(), F_total_1)
        elif np.linalg.norm(robot1_world - cube1_world) < 0.01:
        ...
        robot0_joint_vel = max_velocity * (
            robot0_joint_vel / np.linalg.norm(robot0_joint_vel)
        )
        robot1_joint_vel = max_velocity * (
            robot1_joint_vel / np.linalg.norm(robot1_joint_vel)
        )
...
##tweak threshold to come closer or further to the cube
while (
    np.linalg.norm(robot0_world - platform0_world) > 0.01
    or np.linalg.norm(robot1_world - platform1_world) > 0.01
):
    # Poll positions of end effectors and target
    platform0_world = np.array(get_position(sim, platform_0))
    platform1_world = np.array(get_position(sim, platform_1))
    ...
    F_rep_EE0 = compute_reppotgrad(
        robot1_world, robot0_world, min_thresh=0.001, max_thresh=0.015
    )
    F_rep_platform0_face = compute_reppotgrad(
        platform0_face_world, robot0_world, eps=1, min_thresh=0, max_thresh=0.2
    )
    F_rep_base_0 = compute_reppotgrad(
        robot0_base_world, robot0_world, eps=110, min_thresh=0, max_thresh=0.4
    )
    F_rep_link1_0 = compute_reppotgrad(
        robot_0_link1_midpoint, robot0_world, eps=90, min_thresh=0, max_thresh=0.1
    )
    F_rep_link2_0 = compute_reppotgrad(
        robot_0_link2_midpoint, robot0_world, eps=90, min_thresh=0, max_thresh=0.01
    )
    F_rep_link3_0 = compute_reppotgrad(
        robot_0_link3_midpoint, robot0_world, eps=90, min_thresh=0, max_thresh=0.01
    )

    F_rep_floor1_1 = compute_reppotgrad(
        floor1_world, robot1_world, eps=1, min_thresh=0.01, max_thresh=0.2
    )
    F_rep_floor2_1 = compute_reppotgrad(
        floor2_world, robot1_world, eps=1, min_thresh=0.01, max_thresh=0.2
    )
    F_rep_floor3_1 = compute_reppotgrad(
        floor3_world, robot1_world, eps=1, min_thresh=0.01, max_thresh=0.1
    )
    # F_rep_floor4_1 = compute_reppotgrad(floor4_world, robot1_world, eps=1, min_thresh= 0.01, max_thresh=0.05)
    F_rep_EE1 = compute_reppotgrad(
        robot0_world, robot1_world, min_thresh=0.001, max_thresh=0.015
    )
    F_rep_platform1_face = compute_reppotgrad(
        platform1_face_world, robot1_world, min_thresh=0, max_thresh=0.08
    )
    F_rep_base_1 = compute_reppotgrad(
        robot1_base_world, robot1_world, min_thresh=0, max_thresh=0.1
    )
    F_rep_link1_1 = compute_reppotgrad(
        robot_1_link1_midpoint, robot1_world, min_thresh=0, max_thresh=0.005
    )
    F_rep_link2_1 = compute_reppotgrad(
        robot_1_link2_midpoint, robot1_world, min_thresh=0, max_thresh=0.005
    )
    F_rep_link3_1 = compute_reppotgrad(
        robot_1_link3_midpoint, robot1_world, min_thresh=0, max_thresh=0.005
    )

    # Modifying waypoint sequence
    # F_att_0 = compute_attpotgrad(platform0_world, robot0_world, max_thresh=0.001, eps2 = 100)
    # F_att_1 = compute_attpotgrad(platform1_world, robot1_world, max_thresh=0.001, eps2 = 50)
    # add waypoints here based on
    # F_att_0 = compute_attpotgrad(robot0_waypoint0_world, robot0_world, max_thresh=0.001, eps2 = 100)
    # F_att_1 = compute_attpotgrad(robot1_waypoint0_world, robot1_world, max_thresh=0.001, eps2 = 50)

    # Waypoints for robot0 and robot1
    robot0_waypoints = [
        robot0_waypoint0_world,
        robot0_waypoint1_world,
        robot0_waypoint2_world,
        platform0_world,
    ]

    robot1_waypoints = [
        robot1_waypoint0_world,
        robot1_waypoint1_world,
        robot1_waypoint2_world,
        platform1_world,
    ]

    # Threshold for switching waypoints
    threshold = 0.05
    # Initial max_thresh for attractive force
    initial_max_thresh = 0.5
    increment = 0.5  # Increase max_thresh by 0.5 for each waypoint
    ...
    # Waypoint logic
    if robot0_wp_idx < len(robot0_waypoints):
        current_wp0 = robot0_waypoints[robot0_wp_idx]
        dist_to_wp0 = np.linalg.norm(robot0_world - current_wp0)
        if dist_to_wp0 < threshold:
            print(f"Robot0 reached waypoint {robot0_wp_idx}: {current_wp0}")
            robot0_wp_idx += 1  # Move to the next waypoint
        else:
            # Gradually increase the max_thresh for each waypoint
            max_thresh_0 = initial_max_thresh + robot0_wp_idx * increment
            F_att_0 = compute_attpotgrad(
                current_wp0, robot0_world, max_thresh=max_thresh_0, eps2=100
            )
            print(f"Robot0 attractive force with max_thresh {max_thresh_0}: {F_att_0}")

    if robot1_wp_idx < len(robot1_waypoints):
        current_wp1 = robot1_waypoints[robot1_wp_idx]
        dist_to_wp1 = np.linalg.norm(robot1_world - current_wp1)
        if dist_to_wp1 < threshold:
            print(f"Robot1 reached waypoint {robot1_wp_idx}: {current_wp1}")
            robot1_wp_idx += 1  # Move to the next waypoint
        else:
            # Gradually increase the max_thresh for each waypoint
            max_thresh_1 = initial_max_thresh + robot1_wp_idx * increment
            F_att_1 = compute_attpotgrad(
                current_wp1, robot1_world, max_thresh=max_thresh_1, eps2=50
            )
            print(f"Robot1 attractive force with max_thresh {max_thresh_1}: {F_att_1}")

    # Final checks to determine if navigation is complete
    if robot0_wp_idx >= len(robot0_waypoints) and robot1_wp_idx >= len(
        robot1_waypoints
    ):
        print("Both robots have completed their waypoint navigation.")
    else:
        if robot0_wp_idx < len(robot0_waypoints):
            print(
                f"Robot0 is moving to waypoint {robot0_wp_idx}: {robot0_waypoints[robot0_wp_idx]}"
            )
        if robot1_wp_idx < len(robot1_waypoints):
            print(
                f"Robot1 is moving to waypoint {robot1_wp_idx}: {robot1_waypoints[robot1_wp_idx]}"
            )

    J0 = compute_jacobian(
        robot0_link1_pos, robot0_link2_pos, robot0_link3_pos, 0.352, 0.070, 0.360, 0.574
    )  # account for length of gripper
    J1 = compute_jacobian(
        robot1_link1_pos, robot1_link2_pos, robot1_link3_pos, 0.352, 0.070, 0.360, 0.574
    )
    ...
    quiver_total_0 = ax.quiver(
        ...
        scale=1,
        scale_units="xy",
        label="Total Force Robot 0",
    )
    quiver_total_1 = ax.quiver(
        robot1_world[0],
        robot1_world[2],
        F_total_1[0],
        F_total_1[2],
    ...
    if np.linalg.norm(robot0_world - platform0_world) < 0.01:
        robot0_joint_vel = np.zeros_like(robot0_joint_vel)
        robot1_joint_vel = np.matmul(J1.transpose(), F_total_1)
    elif np.linalg.norm(robot1_world - platform1_world) < 0.01:
        robot0_joint_vel = np.matmul(J0.transpose(), F_total_0)
        robot1_joint_vel = np.zeros_like(robot1_joint_vel)
    else:
        robot0_joint_vel = np.matmul(J0.transpose(), F_total_0)
