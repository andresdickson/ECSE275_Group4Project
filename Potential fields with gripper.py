...
if __name__ == '__main__':
    ...
    while (np.linalg.norm(robot0_world - cube0_world) > 0.05 or np.linalg.norm(robot1_world - cube1_world) > 0.05):
        # print("1")
    ...
    waypoints = [(waypoint0_0, waypoint1_0), (waypoint0_1, waypoint1_1), (waypoint0_2, waypoint1_2)]
    for waypoint in waypoints:
        while (np.linalg.norm(robot0_world - np.array(get_position(sim,waypoint[0]))) > 0.5 or np.linalg.norm(robot1_world - np.array(get_position(sim,waypoint[1]))) > 0.5):
            # Poll positions of end effectors and target
            ...
            if attachedShape0 is None:
                result0, distance0, detectedPoint0, detectedObjectHandle0, detectedSurfaceNormalVector0 = sim.readProximitySensor(objectSensor0)
                if result0 > 0 and detectedObjectHandle0 != -1:
                    # Get the name of the detected object
                    objectName0 = sim.getObjectName(detectedObjectHandle0)
                    print(f"Robot 0 Sensor detected object: {objectName0} (Handle: {detectedObjectHandle0})")
                    
                    # Attach the detected object to the gripper
                    attachedShape0 = detectedObjectHandle0
                    ...
                    print(f"Robot 0 attached object: {objectName0}, new parent: {parent_name}")
                else:
                    print("Robot 0 Sensor did not detect any object.")


                # Gripper code for Robot 1 with debugging and cube attachment
                if attachedShape1 is None:
                    result1, distance1, detectedPoint1, detectedObjectHandle1, detectedSurfaceNormalVector1 = sim.readProximitySensor(objectSensor1)
                    if result1 > 0 and detectedObjectHandle1 != -1:
                        # Get the name of the detected object
                        objectName1 = sim.getObjectName(detectedObjectHandle1)
                        print(f"Robot 1 Sensor detected object: {objectName1} (Handle: {detectedObjectHandle1})")
                        
                        # Attach the detected object to the gripper
                        attachedShape1 = detectedObjectHandle1
                        ...
                        parent_name = sim.getObjectName(parent_handle)
                        print(f"Robot 1 attached object: {objectName1}, new parent: {parent_name}")
                    else:
                        print("Robot 1 Sensor did not detect any object.") 
    ...
    # loops until both arms are close to the cube
    ##tweak threshold to come closer or further to the cube
    while (np.linalg.norm(robot0_world - platform0_world) > 0.2 or np.linalg.norm(robot1_world - platform1_world) > 0.2):
        # Poll positions of end effectors and target
        platform0_world = np.array(get_position(sim,platform_0))
        platform1_world = np.array(get_position(sim,platform_1))
        robot0_world = np.array(get_position(sim, robot0_EE))
        ...
        F_att_0 = compute_attpotgrad(platform0_world, robot0_world, max_thresh=0.1, eps2 = 100)
        F_att_1 = compute_attpotgrad(platform1_world, robot1_world, max_thresh=0.1, eps2 = 100)
        ...
        alpha_att = 1.0
        beta_rep = 1.0
        F_total_0 = -alpha_att * F_att_0 - beta_rep * (F_rep_EE0 + F_rep_link1_0 + F_rep_link2_0 + F_rep_link3_0 + F_rep_base_0 + F_rep_platform0_face)
        # print(F_total_1)
        # F_total_0[1] = 0
        F_total_1 = -alpha_att * F_att_1 - beta_rep * (F_rep_EE1 + F_rep_link1_1 + F_rep_link2_1 + F_rep_link3_1 + F_rep_base_1 + F_rep_platform1_face + F_rep_floor1_1)
        # F_total_1[1] = 0
        ...
        if np.linalg.norm(robot0_world - platform0_world) < 0.2:
            robot0_joint_vel = np.zeros_like(robot0_joint_vel)
            robot1_joint_vel = np.matmul(J1.transpose(), F_total_1)
        elif np.linalg.norm(robot1_world - platform1_world) < 0.2:
            robot0_joint_vel = np.matmul(J0.transpose(), F_total_0)
            robot1_joint_vel = np.zeros_like(robot1_joint_vel)
        else:
            robot0_joint_vel = np.matmul(J0.transpose(), F_total_0)
