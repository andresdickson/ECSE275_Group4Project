...
if __name__ == '__main__':
    ...
    waypoint1_0 = sim.getObjectHandle("/waypoint0_0")
    waypoint1_1 = sim.getObjectHandle("/waypoint0_1")
    waypoint1_2 = sim.getObjectHandle("/waypoint0_2")
...
# loops until both arms are close to the cube
##tweak threshold to come closer or further to the cube
while (np.linalg.norm(robot0_world - platform0_world) > 0.3 or np.linalg.norm(robot1_world - platform1_world) > 0.25):
    # Poll positions of end effectors and target
    platform0_world = np.array(get_position(sim,platform_0))
    platform1_world = np.array(get_position(sim,platform_1))
    ...
    F_rep_base_0 = compute_reppotgrad(robot0_base_world, robot0_world, eps=10,  min_thresh= 0, max_thresh=0.7) #0.7 works
    ...
    F_rep_platform1_face = compute_reppotgrad(platform1_face_world, robot1_world, min_thresh= 0, max_thresh=0.25)
